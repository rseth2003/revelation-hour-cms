<x-admin-layout title="RHMI CMS Dashboard" heading="Dashboard">
    <section class="mb-8 rounded-2xl bg-gradient-to-r from-[#072f68] to-[#0d5fa8] p-6 text-white shadow-lg">
        ...
        <p class="mt-2 max-w-2xl text-blue-100">Manage every completed website module and track what we are building next.</p>
    </section>
        ...
        @foreach([
            ...
            <article class="rounded-2xl border border-blue-200 bg-white p-5 shadow-sm">
                ...
                <a href="{{ route($routeName) }}"
                   class="mt-5 block w-full rounded-lg bg-[#072f68] px-4 py-2 text-center text-sm font-semibold text-white">
                    Manage
                </a>
            </article>
        @endforeach
    ...
    <section class="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
        @foreach([
            ['Prayer Requests','Receive and manage public prayer requests.','♡','bg-pink-100 text-pink-700'],
            ...
            ['Users and Roles','Control access for pastors, admins and media teams.','♟','bg-slate-200 text-slate-700'],
            ['Analytics','Website activity and CMS content statistics.','▥','bg-emerald-100 text-emerald-700'],
        ] as [$title,$description,$icon,$accent])
            <article class="rounded-2xl border bg-white p-5 shadow-sm">
                <div class="mb-4 flex justify-between">
                    <span class="grid h-12 w-12 place-items-center rounded-xl text-xl {{ $accent }}">{{ $icon }}</span>
                    <span class="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-500">Coming Soon</span>
                </div>

                <h3 class="text-lg font-bold text-[#072f68]">{{ $title }}</h3>
                <p class="mt-2 text-sm leading-6 text-slate-600">{{ $description }}</p>
                <button disabled class="mt-5 w-full cursor-not-allowed rounded-lg bg-slate-100 px-4 py-2 text-sm font-semibold text-slate-400">Not Available Yet</button>
            </article>
        @endforeach
    </section>
</x-admin-layout>
