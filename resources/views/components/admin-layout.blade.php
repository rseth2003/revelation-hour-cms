...
<div class="min-h-screen lg:flex">
    <aside id="adminSidebar" class="fixed inset-y-0 left-0 z-40 w-72 -translate-x-full bg-[#072f68] text-white transition-transform duration-200 lg:static lg:translate-x-0">
        <div class="flex h-20 items-center gap-3 border-b border-white/10 px-6">
            ...
            <div>
                <p class="font-bold tracking-wide">RHMI CMS</p>
        ...
        <nav class="space-y-1 p-4 text-sm">
            <a href="{{ route('dashboard') }}"
               class="flex items-center gap-3 rounded-lg px-4 py-3 font-semibold hover:bg-white/10 {{ request()->routeIs('dashboard') ? 'bg-white/15' : '' }}">
                <span>▦</span><span>Dashboard</span>
            </a>

            <a href="#" class="flex items-center gap-3 rounded-lg px-4 py-3 hover:bg-white/10">
                <span>◉</span><span>Word of the Day</span>
            </a>

            <a href="{{ route('admin.events.index') }}"
               class="flex items-center gap-3 rounded-lg px-4 py-3 hover:bg-white/10 {{ request()->routeIs('admin.events.*') ? 'bg-white/15' : '' }}">
                <span>▣</span><span>Live Events</span>
            </a>

            <a href="{{ route('admin.ministries.index') }}"
               class="flex items-center gap-3 rounded-lg px-4 py-3 hover:bg-white/10 {{ request()->routeIs('admin.ministries.*') ? 'bg-white/15' : '' }}">
                <span>◆</span><span>Ministries</span>
            </a>

            <a href="#" class="flex items-center gap-3 rounded-lg px-4 py-3 hover:bg-white/10">
                <span>▶</span><span>Sermons</span>
            </a>
        ...
        <div class="absolute bottom-0 left-0 right-0 border-t border-white/10 p-4">
            <a href="{{ route('home') }}"
               class="block rounded-lg border border-white/20 px-4 py-3 text-center text-sm hover:bg-white/10">
                View Public Website
            </a>
        </div>
    </aside>
        ...
        <header class="sticky top-0 z-30 flex h-20 items-center justify-between border-b bg-white px-4 shadow-sm sm:px-6">
            ...
            <div class="flex items-center gap-3">
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="rounded-lg bg-[#072f68] px-3 py-2 text-sm font-semibold text-white hover:bg-blue-900">
                        Logout
                    </button>
                </form>
            </div>
